...
*   **`activityLogs` (Motor de Proactividad):** Esta tabla es un registro histórico del estilo de vida del usuario. Su función principal es guardar eventos (ej. "Entrenamiento", "Fiesta") asociados a un peso o intensidad. El objetivo de esta tabla es que el motor la lea para inferir automáticamente qué tipo de ropa sucia se está generando y escalar el `priorityScore` correspondiente.
    *   *Posibles integraciones futuras:* Se puede construir una interfaz manual sencilla para que el usuario registre estas actividades. Sin embargo, para lograr un enfoque "Zero-Friction" en un ecosistema Android/Windows, se sugiere integrar **Health Connect API** (que lee los pasos o entrenamientos del celular, incluso sin smartwatch) o exponer un Webhook para **Samsung Routines / Tasker** (ej. disparar un evento por geolocalización al llegar a casa tras el gimnasio).
